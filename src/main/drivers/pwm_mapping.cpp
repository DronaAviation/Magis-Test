...
static const uint16_t multiPWM[] = {
    ...
    PWM14 | (MAP_TO_MOTOR_OUTPUT << 8),             //M1 for V3R
    PWM13 | (MAP_TO_MOTOR_OUTPUT << 8),//M2 for V3R
    PWM2 | (MAP_TO_MOTOR_OUTPUT << 8),//M3 for V3R
    PWM7 | (MAP_TO_MOTOR_OUTPUT << 8),//M4 for V3R
    0xFFFF
};
...
#if defined(SPARKY) || defined(ALIENWIIF3) || defined(PRIMUSX) || defined(PRIMUSV4)
static const uint16_t multiPPM[] = {
...
static const uint16_t * const hardwareMaps[] = { multiPWM, multiPPM, airPWM, airPPM, };
...
#ifdef SONAR
        if (init->sonarGPIOConfig && timerHardwarePtr->gpio == init->sonarGPIOConfig->gpio && (timerHardwarePtr->pin == init->sonarGPIOConfig->triggerPin || timerHardwarePtr->pin == init->sonarGPIOConfig->echoPin)) {
            continue;
        }
#endif
